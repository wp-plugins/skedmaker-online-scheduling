...
if(!function_exists('SM_noshow_check')){function SM_noshow_check(){
	...
	if($_GET['op']=='noshow'){
		$usercode=$_GET['uc'];
		$result=mysql_query("SELECT * FROM skedmaker_clients WHERE usercode='$usercode'");
		while($row = mysql_fetch_array($result)) {
			$email=SM_d($row['email']);
			$name=SM_d($row['first'])." ".SM_d($row['last']);
		}
		$result=mysql_query("SELECT * FROM skedmaker_sked WHERE usercode='$usercode'");
		while($row = mysql_fetch_array($result)) {
			$start_date=$row['startdate'];
			$already_noshow=$row['noshow'];
			if($start_date!=""){$apt=SM_apt($start_date);}
		}
...
if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=='noshow_confirm'){
		...
		$result=mysql_query("SELECT * FROM skedmaker_clients WHERE usercode='$usercode'");
		while($row = mysql_fetch_array($result)) {
			$current_noshows=$row['noshows'];
			$email=SM_d($row['email']);
		}

		$current_noshows++;
		$saveIt=mysql_query("UPDATE skedmaker_clients SET noshows='$current_noshows' WHERE usercode='$usercode'");
		if(!$saveIt){
			SM_redBox("Error, try again later.", 400, 21);
		}else{
			$saveIt=mysql_query("UPDATE skedmaker_sked SET noshow='y' WHERE usercode='$usercode'");
			if(!$saveIt){
				SM_redBox("Error, try again later.", 400, 21);
			}else{
				SM_greenBox("Marked No Show!", 400, 21);
				SM_redirect($redirect_page, 500);
				die();
			}
		}
	}
...
function SM_apt_output($this_page, $dayTS, $time_start, $time_end, $dayCheck, $remaining, $total_blocked, $time_passed, $appointmentAvailable, $appointmentUnavailable, $publicschedule, $stagger){
	...
	// ======= output 
	echo $tr_data;
	echo "<td style='border:0px;' class='".$td_class."'>";
	echo $div;
...
if(!function_exists('SM_remove_custom_from_day')){function SM_remove_custom_from_day($customskedcode, $bulkReturn, $year, $month, $day, $weekday){
	...
	if($_GET['op']=='removecustom'){
		if ($_SERVER['REQUEST_METHOD']=='POST'){
			...
			if(!$saveIt){
				SM_redBox("Could not remove the custom setting for this day, try again later.", 600, 16);
			}else{
				echo"<br>";
				SM_greenBox("Removed custom setting!", 800, 21);
				if($_GET['blkrtn']==""){
		...
		if($total>0){ 
			...
			<?php SM_redBox("Are you SURE you want to remove the custom setting from this date?", 750, 16); ?>
			</td></tr></table>
			<table class='cc100'>
	...
	<?php 
			...
			SM_redBox("No custom template set to this date.", 800, 21);
			die();
		}
		echo "</td></tr></table>";
...
$startData="<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">
	...
	table.cc100{border-collapse:collapse; border-spacing:0; width:100%;}
	table.cc800{border-collapse:collapse; border-spacing:0; width:800px;}
...
if(!function_exists('SM_cancel_apt')){function SM_cancel_apt($isAdmin){
	...
	if($_GET['op']=="cancel"){
		...
		if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=="cancel"){
			...
			if(!$cancelIt){
				SM_redBox("Unable to cancel appointment, try again later.", 600, 21);
			}else{
				echo"<br>";
				SM_greenBox("Appointment Cancelled!", 600, 21);
				...
				if(SM_emailIt($canEmail, $adminemail, $adminemail, "Appointment Cancelled: ".$canName, $bodyData)!=true){
					SM_redBox("Cancellation e-mail could not be sent.", 800, 21);
				}
...
if(!function_exists('SM_purge_past_confirm')){function SM_purge_past_confirm(){
	if ($_SERVER['REQUEST_METHOD']=='POST' && $_GET['op']=='purge_confirm'){
		...
		if(!$saveIt){
			SM_redBox("Could not purge. Try again later.", 800, 21);
		}else{
			SM_greenBox("Purged All Past Appointments!", 800, 21);
...
if(!function_exists('SM_foot')){function SM_foot(){
    ...
    <table style='width:100%; border:0px; margin-top:14px;'><tr><td class='pad7' style='text-align:center;'><span class='smallG' style='font-weight:normal;'>Skedmaker WordPress Plugin version .78 © Copyright Skedmaker Online Scheduling</span></td></tr></table>
	<?php 
